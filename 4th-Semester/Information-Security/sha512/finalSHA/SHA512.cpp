...
const int64 SHA512::Constants[80]= {
...
string SHA512::getHexFrom4bit(string bin){ // Convert 4digit binary string to hex string format.
...
string SHA512::getHexFromDecimal(int64 deci){
...
int64 SHA512::getDecFromBin(string bin){
...
void SHA512::message_schedeuler(string single_block){
  int nChunk = 0;
...
int64 SHA512::sum_1(int64 e){
  return rotate_right(e, 14) ^ rotate_right(e, 18) ^ rotate_right(e, 41);
}

int64 SHA512::sum_0(int64 a){
  return rotate_right(a, 28) ^ rotate_right(a, 34) ^ rotate_right(a, 39);
}

void SHA512::round(int64 a, int64 b, int64 c,int64& d, int64 e, int64 f,int64 g, int64& h, int K){
...
string SHA512::hash(string msg){
  ...
  string lengthbits = std::bitset<128>(msglen_inbits).to_string();
  ...
  for (int letsgo = 0; letsgo < nBlock; ++letsgo){
    ...
    for (int i = 0; i < 80; i++) { //REQUIRES EDITTTTT
